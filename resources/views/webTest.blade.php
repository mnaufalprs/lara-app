            ...
            <form action="/webTest" method="post">
                ...
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    ...
                    <div class="w-full" style="visibility: hidden">
                        ...
                        <div type="number" name="user_id" id="user_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="" required="">{{ $dataUserId }}</div>
                    </div>
                ...
                <div class="inline-flex items-center py-2.5">
                    <a href="/webAnalytic" class="inline-flex items-center py-2.5 px-5 me-2 mb-2 ml-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                        View Result
...
<div class="border-2 border-dashed rounded-lg border-gray-100 dark:border-gray-800 h-auto mb-4 dark:bg-gray-800">
...
const dataUserIdElement = document.getElementById('user_id');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addTestingButton.addEventListener('click', function() {
        ...
        // Lakukan tindakan lain yang diperlukan (misalnya, kirim permintaan HTTP)

        // Misalnya, Anda bisa mengarahkan ulang halaman atau melakukan tindakan lain.
        // window.location.reload(); // Contoh: mengarahkan ulang halaman

        // Anda bisa menambahkan kode lainnya di sini, jika diperlukan.
    });
    ...
    stopTestingButton.addEventListener('click', function() {
        ...
        // Lakukan tindakan lain yang diperlukan untuk menghentikan proses
        // Misalnya, emit event ke server untuk menghentikan testing
        
        // Lakukan tindakan lainnya di sini, jika diperlukan.
    });
});
...
function handleData(type, data) {
    ...
    const datawbtest = JSON.parse(localStorage.getItem('datawbtest'));
    const datawbtestinp = JSON.parse(localStorage.getItem('datawbtestinp'));

    if (datawbtest && datawbtestinp) {
        // Periksa kecocokan datawbtest dengan datawbtestinp
        const matchFound = checkMatch(datawbtest, datawbtestinp);
...
function checkMatch(datawbtest, datawbtestinp) {
    for (const wbtest of datawbtest) {
        for (const wbtestinp of datawbtestinp) {
            if (
                wbtest.user_id === wbtestinp.user_id &&
                wbtest.server_address === wbtestinp.server_address &&
                wbtest.input_test_id === wbtestinp.test_id
            ) {
...
socket.on('datawbtest', (datawbtest) => {
    handleData('datawbtest', datawbtest);
});

socket.on('datawbtestinp', (datawbtestinp) => {
    handleData('datawbtestinp', datawbtestinp);
});
